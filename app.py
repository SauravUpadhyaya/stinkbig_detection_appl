...
IMPORTANT RULES:
- If user mentions a specific place (city, state, country), ALWAYS set query_type to "specific_location"
- Extract the exact location name from the user's question
- If asking "how many bugs in [LOCATION]?", that's a specific_location query
- Default metric to "total_count" if not specified

Return ONLY valid JSON, no other text:
{{"query_type": "...", "comparison_type": null, "metric": "total_count", "location": "...", "time_period": null}}
...
JSON:"""
    ...
    if not intent:
        ...
        # Extract location if present (e.g., "baton rouge", "california", "louisiana")
        ...
        # Try to find US state in question
        words = query_lower.split()
        for i, word in enumerate(words):
            word_clean = word.rstrip('?.,!;:')
            if word_clean in us_states:
                extracted_location = word_clean.title()
                break
            # Also try two-word states like "new york"
            if i < len(words) - 1:
                two_word = (word_clean + " " + words[i+1].rstrip('?.,!;:')).lower()
                if two_word in us_states:
                    extracted_location = two_word.title()
                    break
        
        # Also try to extract city/location after prepositions
        if not extracted_location:
            for prep in [" in ", " from ", " at "]:
                if prep in query_lower:
                    after_prep = query_lower.split(prep)[-1].strip()
                    location_words = after_prep.split()
                    for length in range(min(3, len(location_words)), 0, -1):
                        candidate = " ".join(location_words[:length]).rstrip('?.,!;:')
                        if candidate and len(candidate) > 2:
                            extracted_location = candidate.title()
                            break
        ...
        if any(word in query_lower for word in ["how many", "how much", "count", "total", "number of", "many"]):
            if any(word in query_lower for word in ["cities", "locations", "places", "areas"]):
                ...
                # If location is mentioned, treat as specific_location query
                intent = {
                    "query_type": "specific_location",
                    "comparison_type": None,
                    "metric": "total_count",
